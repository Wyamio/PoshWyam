function Install-Wyam {
    ...
    begin {
        ...
        $wyam = Join-PathSegment $tools "wyam","tools","wyam.exe"
    }
    ...
    process {
        ...
        if (-not (Test-Path $nuget)) {
            ...
            if ($null -ne $found -and (Test-Path $found.FullName)) {
        ...
        if (-not (Test-Path $wyam)) {
            ...
            [void](Invoke-Expression "&`"$nuget`" install Wyam -Prerelease -ExcludeVersion -OutputDirectory `"$tools`"")
        }
    }
